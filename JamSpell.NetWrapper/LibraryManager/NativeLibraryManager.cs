...
namespace JamSpell.NetWrapper.LibraryManager
{
	...
	{
		private static readonly ILog _log = LogProvider.For<NativeLibraryManager>();
		...
		public NativeLibraryManager(string name, params LibraryItem[] items)
		{
			_name = name;
			_items = items;
		}
		...
		private LibraryItem FindItem()
		{
			...
			var item = _items.FirstOrDefault(x => x.Os == platform && x.Bitness == bitness);
			if (item == null)
			{
				throw new InvalidOperationException($"There is no supported native library for platform '{platform}' and bitness '{bitness}'");
			}
		...
		private Os GetPlatform()
		{
			...
			else if (File.Exists(@"/proc/sys/kernel/ostype"))
			{
				...
				{
					throw new InvalidOperationException($"Unsupported OS: {osType}");
				}
			}
